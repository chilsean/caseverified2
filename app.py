...
try:
    ...
    if not tesseract_path:
        raise FileNotFoundError("Tesseract OCR is missing. Ensure `apt.txt` contains `tesseract-ocr`.")

    # Set the Tesseract path
    pytesseract.pytesseract.tesseract_cmd = tesseract_path

except (ImportError, FileNotFoundError):
    st.error("Tesseract OCR is missing. Ensure `apt.txt` contains `tesseract-ocr`. Then redeploy the app.")
    st.stop()
